class MapPopulator{
    ...
    populate_with_marks(location_type, location_id, x, y){
        let behind = this.map.get.inspector.entity.fetch_behind_shop(x, y);
        let shop = this.map.get.inspector.entity.fetch_shop(location_type, location_id, x, y);
    ...
    populate_with_trash_cans(id){
        ...
        let num_of_trash_cans = rand_num(Math.round(size * .01), Math.round(size * .05));
