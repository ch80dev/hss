class MapGenerator {
    ...
    generate_lights(){
        ...
        if (this.map.location.type == 'alley'){
            this.generate_lights_for_alley();
            return;
        }
        this.generate_lights_for_street();
    }

    generate_lights_for_alley(){
        ...
        while(n < 100){
            let good = null;
            border = this.map.queries.fetch_open();
            for (let spot of lights){
                let distance = this.map.queries.fetch_distance(spot.x, spot.y, border.x, border.y);
    ...
    generate_lights_for_street(){
        ...
        while(n < 1000){
            ...
            let open = this.map.queries.fetch_open_with_distance(rand_spot.x, rand_spot.y, 4);
            ...
            if (n == 0 && (good.x  >= Config.max_x - 5 && good.x < 5 && !valid_x.includes(good.x))){
                valid_x.push(good.x);
            } else if (n == 0 && (good.y  >= Config.max_y - 5 && good.y < 5 && !valid_y.includes(good.y)) ){
                valid_y.push(good.y);
