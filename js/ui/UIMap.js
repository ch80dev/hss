    ...
    display(){
		...
		for (let y = 0; y < Config.max_y; y ++){
			...
			for (let x = 0; x < Config.max_x; x ++){								
				...
				let human = juego.fetch_human_by_loc(juego.player.state.location.type, juego.player.state.location.id, x, y);
				...
				let mark = juego.map.queries.fetch_mark(juego.player.state.location.type, juego.player.state.location.id, x, y);
				let rat = juego.fetch_rat(juego.player.state.location.type, juego.player.state.location.id, x, y);
				let is_lit = false;
				...
				// 3. The "Darkness" Skip
				if (juego.night && !is_lit && !juego.map.queries.is_in_the_light(juego.player.state.location.type, juego.player.state.location.id, x, y)) {
