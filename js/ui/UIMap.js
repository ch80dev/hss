    ...
    display(){
		...
		for (let y = 0; y < MapConfig.max_y; y ++){
			...
			for (let x = 0; x < MapConfig.max_x; x ++){								
				...
				// 3. The "Darkness" Skip
				if ((juego.player.state.location.type ==  'sewer' || juego.night) && !is_lit && !juego.map.get.inspector.entity.is_in_the_light(juego.player.state.location.type, juego.player.state.location.id, x, y) && ((map_at != null && map_at != 1) || (rand_num(1, 15) == 1))) {
