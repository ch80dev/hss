class UIHuman{
		...
		for (let id in human.interactions){			
			...
			if (typeof human.resources[id]  == 'object' && interaction == 'trade'){
				...
				if (interaction == 'sell' && !ItemConfig.stackable.includes(human.resources[id])){
					button = this.display_unique_items_to_sell(human.resources[id], id);
	...
	display_unique_items_to_sell(name, interaction_id){
		...
		for (let id of item_ids){
			...
			txt += `<button id='sell_unique_to_human-${id}-${interaction_id}' class='sell_unique_to_human  button_${this.button_num}'>sell ${item.name} (${item.durability}%)</button>`;
			this.button_num ++;
		}
		return txt;
	}
