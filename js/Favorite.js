class Favorite{
    ...
    add(type, id, juego){
		...
		if (this.set[type][id] == undefined){
			this.set[type][id] = { location: { type: juego.player.state.location_type, id: juego.player.state.location_id }, x: favorite_target.x, y: favorite_target.y, path: {},  }
		...
		for (let entity of Object.keys(relevant_favorites)){
			for (let  relevant of relevant_favorites[entity]){
                ...
                if (this.is_here(to) && this.is_here(from)){
                    this.remove(from, to);
                } else if (spot_id == null || favorite.path[to] == undefined){
					favorite.path[to] = [from];					
                } else if (favorite.path[to] != undefined){
                    favorite.path[to].push(from);
				}
